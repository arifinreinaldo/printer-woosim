package net.simplr.woosimdp230l.sunmi;
...
import net.simplr.woosimdp230l.R;
    ...
    public static boolean connectBlueTooth(Context context) {
        if (bluetoothSocket == null) {
            if (getBTAdapter() == null) {
                Toast.makeText(context, "Bluetooth adapter empty", Toast.LENGTH_SHORT).show();
                return false;
            }
            if (!getBTAdapter().isEnabled()) {
                Toast.makeText(context, "Bluetooth adapter is not enabled", Toast.LENGTH_SHORT).show();
                return false;
            }
            BluetoothDevice device;
            if ((device = getDevice(getBTAdapter())) == null) {
                Toast.makeText(context, "Bluetooth adapter wrong", Toast.LENGTH_SHORT).show();
                return false;
            }
            ...
            try {
                ...
                Toast.makeText(context, "Bluetooth adapter fail to connect", Toast.LENGTH_SHORT).show();
                return false;
            }
        }
        return true;
